...
class TwoTowerNNVSModel(nn.Module):
    """
    Two-Tower Neural Network Virtual Source Model (copied locally for SiFET).
...
def _load_twotower_model(embed_size=16):
    """Load the pretrained Two-Tower model for SiFET inference."""
    model = TwoTowerNNVSModel(embed_size=embed_size, charge_include=True)
    model_dir = os.path.dirname(__file__)
    model_path = os.path.join(model_dir, "SiFET_idvd_idvg.pth")
    if not os.path.exists(model_path):
        raise FileNotFoundError(
            f"Pretrained weights not found at {model_path}. "
            "Place SiFET_idvd_idvg.pth alongside SiFET.py."
        )
...
def run_twotower_sim(parameters, config=None):
    ...
    Vd_mesh, Vg_mesh = torch.meshgrid(Vd_tensor, Vg_shift_list, indexing='ij')
